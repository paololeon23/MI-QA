/** Dashboard resumen todas las fechas — Plagas Uva (port mostrarResumenTodasFechas) */

import { renderNestedErrorTableHtml } from "./uva-plagas-table.js";
import { buildDuplicateAlertHtml } from "./uva-plagas.validation.js";
...
export function mountReviewAllDashboard(
  ...
  const withIssues = items.filter(
    (item) => item.errorRows?.length || item.lotesDuplicados?.length
  );
  ...
  withIssues.forEach((item) => {
    ...
    const tableHtml = item.errorRows?.length
      ? renderNestedErrorTableHtml(item.errorRows, headers, config, columnLabelsByIndex, htmlEscape, t)
      : "";
    const dupOnlyHtml = !item.errorRows?.length && item.lotesDuplicados?.length
      ? `<div class="agv-mp-dialog__html agv-mp-dialog__html--stacked">${buildDuplicateAlertHtml(item.lotesDuplicados)}</div>`
      : "";
    section.innerHTML = `
