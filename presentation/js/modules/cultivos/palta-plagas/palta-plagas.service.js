...
export class PaltaPlagasService {
  constructor() {
    this.shell = null;
    this.config = null;
    this.reglas = null;
  ...
  async init(appRoot) {
    ...
    if (!configRes.ok) throw new Error("No se pudieron cargar las reglas de Plagas Palta");
  ...
  async onFileSelected(event) {
    ...
    try {
      ...
      showMpDialog({
        icon: "success",
        title: "Excel cargado",
        html: `Cartilla <b>${htmlEscape(this.config?.cartilla || "EPPP")}</b> · <b>${this.rawData.length}</b> registros · <b>${this.config?.total_columnas || 0}</b> columnas`,
        timer: 1800,
  ...
  populateFechasSelect() {
    ...
    if (!sel) return;
    sel.innerHTML = `<option value="" disabled selected>${t("plagasPalta.selectDate")}</option>`;
    fechas.forEach((f) => sel.add(new Option(f, f)));
    sel.disabled = fechas.length === 0;

    const cosechaSel = this.shell.refs.cosechaSelect;
    if (cosechaSel) {
      cosechaSel.innerHTML = `<option value="" selected>${htmlEscape(t("plagasPalta.autoDate"))}</option>`;
  ...
  syncHarvestDateField() {
    ...
    const row = this.rawData.find((r) => cellDisplayValue(r[fechaIdx]) === fecha);
    const cosecha = row ? cellDisplayValue(row[cosechaIdx]) : "";

    cosechaSel.replaceChildren();
    const opt = document.createElement("option");
    opt.value = cosecha;
    opt.textContent = cosecha || t("plagasPalta.autoDate");
    opt.selected = true;
  ...
  async onRunReview() {
    ...
    const { errorCount, totalRows } = renderPaltaPlagasTable({
    ...
    if (this.shell.refs.resultsTable) this.shell.refs.resultsTable.hidden = false;
        ...
        <div class="${p("excel-insight__top")}">
          ...
          <div class="${p("excel-insight__ring")}" aria-hidden="true">
            <svg viewBox="0 0 100 100" shape-rendering="geometricPrecision">
              <defs>
                <linearGradient id="paltaPlagasInsightRingGrad" x1="0%" y1="0%" x2="100%" y2="100%">
  ...
  async onExport() {
    ...
    if (!this.processedData.length || !fechaActual) {
      showMpDialog({
        icon: "error",
        title: t("plagasArandano.error"),
        text: t("plagasEsparrago.noExportData")
      });
      return;
    }
    ...
    const result = await new Promise((resolve) => {
      ...
      overlay.innerHTML = `
        <div class="agv-mp-dialog agv-mp-dialog--wide">
          ...
          <div class="agv-mp-dialog__html" id="agv-palta-plagas-export-body">${buildExportHtml()}</div>
      ...
      const bodyEl = overlay.querySelector("#agv-palta-plagas-export-body");
    ...
    if (result === "single") {
      writePlagasExportFile(
        this.processedData,
        this.headers,
        `Export_Plaga_Palta_${String(fechaActual).replaceAll("/", "-")}.xlsx`
      );
    } else if (result === "merge") {
      const rows = this.rawData.filter((r) =>
        fechasDisponibles.includes(cellDisplayValue(r[fechaIdx]))
      );
      writePlagasExportFile(rows, this.headers, "Export_Plaga_Palta_Fechas_Unidas.xlsx");
    }
  }
