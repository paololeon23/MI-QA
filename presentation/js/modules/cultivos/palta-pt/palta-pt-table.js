/** Tabla Palta PT — acciones, E_C, filtros, sticky, menú columnas */
...
function buildActionsCell(row, tr, fechaEmbalaje, onRowMark, onCopyReport) {
  ...
  copyBtn.addEventListener("click", (e) => {
    e.preventDefault();
    e.stopPropagation();
    onCopyReport(row, fechaEmbalaje);
  });
...
function renderBodyRows(tbody, rows, headers, tableEl, fechaEmbalaje, onRowMark, onCopyReport) {
  tbody.innerHTML = "";
  rows.forEach((row, rowIndex) => {
    ...
    tr.appendChild(buildActionsCell(row, tr, fechaEmbalaje, onRowMark, onCopyReport));

    const trazCol = getColTrazabilidadJs();
    const ext = { E_C: deriveEtapaCampo(row[trazCol]) };
    ...
    getColumnasFront().forEach((col, displayIdx) => {
      ...
      if (col === "E_C") {
        td.textContent = ext.E_C ?? "";
      } else {
        td.textContent = formatCellDisplay(row[col], col);
        applyCellValidation(td, row, col, row[col], fechaEmbalaje);
...
export function bindPaltaPtColumnMenu(tableEl, menuEl) {
...
export function renderPaltaPtTable({
  headerRow,
  bodyRows,
  headers,
  tableEl,
  ...
  const filterDestino = getFilterDestinoCol();
  const filterFormato = getFilterFormatoCol();

  getColumnasFront().forEach((col, displayIdx) => {
    const th = document.createElement("th");
    if (col === filterDestino) {
      buildFilterHeader(th, col, displayIdx, "Destino", "agv-pt-filter-destino");
    } else if (col === filterFormato) {
      buildFilterHeader(th, col, displayIdx, "Formato", "agv-pt-filter-formato");
  ...
  const refreshFilters = (dataActual, destinoVal, formatoVal) => {
    const destinoSel = document.getElementById("agv-pt-filter-destino");
    const formatoSel = document.getElementById("agv-pt-filter-formato");
    const dataParaDestino = formatoVal === "TODOS" ? allRowsForDate : dataActual;
    const dataParaFormato = destinoVal === "TODOS" ? allRowsForDate : dataActual;
    const destinos = [
      ...new Set(dataParaDestino.map((r) => String(r[filterDestino] ?? "").trim()))
    ]
      .filter(Boolean)
      .sort();
    const formatos = [
      ...new Set(dataParaFormato.map((r) => String(r[filterFormato] ?? "").trim()))
    ]
      .filter(Boolean)
      .sort();
    populateFilterSelect(destinoSel, destinos, destinoVal);
    populateFilterSelect(formatoSel, formatos, formatoVal);
  };

  const paint = (rows) => {
    renderBodyRows(bodyRows, rows, headers, tableEl, fechaEmbalaje, null, onCopyReport);
    bindCopyableCells(bodyRows);
    bindRowDragDrop(bodyRows, rows, onReorder);
  };

  const ejecutarFiltro = () => {
    const destinoVal = document.getElementById("agv-pt-filter-destino")?.value || "TODOS";
    const formatoVal = document.getElementById("agv-pt-filter-formato")?.value || "TODOS";
    const dataFinal = allRowsForDate.filter((r) => {
      const matchDest =
        destinoVal === "TODOS" || String(r[filterDestino] ?? "").trim() === destinoVal;
      const matchFor =
        formatoVal === "TODOS" || String(r[filterFormato] ?? "").trim() === formatoVal;
      return matchDest && matchFor;
    });
    paint(dataFinal);
    refreshFilters(dataFinal, destinoVal, formatoVal);
  ...
  if (!tableEl?.dataset.filtersBound) {
    ...
    document.getElementById("agv-pt-filter-destino")?.addEventListener("change", ejecutarFiltro);
...
export function buildWhatsappReport(row, fechaEmbalaje) {
  const incidencias = collectRowIncidencias(row, fechaEmbalaje);
